...
- Pin/star sessions to the top of the sidebar (gold indicator). Pins are stored in hermes-agent's `state.db` (`sessions.pinned`), the same record Hermes Desktop and `hermes sessions pin` use, so a pin made in any of them shows in all of them; the WebUI's per-session sidecar only caches the flag and is overwritten from `state.db` on each sidebar build. Pinning returns `503` if that record cannot be written, so a pin is never reported as saved when Desktop cannot see it. Sessions that exist only in the WebUI (no `state.db` row) keep their pin in the sidecar.
